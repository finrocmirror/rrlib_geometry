...
const typename tSplineCurve<Tdimension, TElement, 3>::tBezierCurve tUniformBSplineCurve<Tdimension, TElement>::GetBezierCurveForSegment(unsigned int i) const
{
  ...
  bezier_control_points.push_back((1.0 - this->tension) / 4.0 *(this->GetControlPoint(i) + this->GetControlPoint(i + 2)) + (1.0 + this->tension) / 2.0 * this->GetControlPoint(i + 1));
  bezier_control_points.push_back((1.0 + this->tension) / 2.0 * this->GetControlPoint(i + 1) + (1.0 - this->tension) / 2.0 * this->GetControlPoint(i + 2));
  bezier_control_points.push_back((1.0 - this->tension) / 2.0 * this->GetControlPoint(i + 1) + (1.0 + this->tension) / 2.0 * this->GetControlPoint(i + 2));
  bezier_control_points.push_back((1.0 - this->tension) / 4.0 *(this->GetControlPoint(i + 1) + this->GetControlPoint(i + 3)) + (1.0 + this->tension) / 2.0 * this->GetControlPoint(i + 2));
